...
public class FileUtil  extends org.apache.commons.io.FileUtils{
	 ...
	 * @throws Exception
	 */
	public String uploadFile(MultipartFile file, String baseDir, String suffix)
			throws Exception {
		String uuid = UUID.randomUUID().toString();
		StringBuilder sb = new StringBuilder();
		sb.append(DateUtils.getYear());
		sb.append(File.separator).append(DateUtils.getMonth());
		sb.append(File.separator).append(DateUtils.getDay());
		sb.append(File.separator).append(uuid).append(suffix);
		String fileName = sb.toString();
		File localFile = new File(baseDir + sb.toString());
		log.debug("文件保存的完整路径为：" + localFile.getAbsolutePath());
	...
	/**
	 *
	 * @description 根据size传入大小自动处理成GB MB KB单位
	 * @param size
	 * @return String 1GB 1MB 1KB
	 */
